...
export async function submitToN8n(rawFormData, options = {}) {
  ...
  const targetUrl = N8N_TEST_WEBHOOK_URL;

  console.info('[MediClin] Dispatching HTTP POST to n8n Test Webhook:', targetUrl);
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 404 && (errorJson?.message?.includes('not registered') || errorText.includes('not registered') || errorJson?.hint?.includes('test') || errorJson?.hint?.includes('active'))) {
        return {
          success: false,
